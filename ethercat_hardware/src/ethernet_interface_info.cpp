...
void EthernetInterfaceInfo::initialize(const std::string &interface) {
  ...
  if (sock_ < 0) {
    SHADOWHAND_WARN("Cannot get control socket for ioctls : %s",
                    strerror(errno));
    return;
  }
  ...
  if (result < 0) {
    SHADOWHAND_WARN("Cannot get driver information for %s : %s",
                    interface_.c_str(), strerror(errno));
    return;
  }
  n_stats_ = drvinfo.n_stats;
  if (n_stats_ < 1) {
    SHADOWHAND_WARN("No NIC statistics available for %s", interface_.c_str());
    return;
  }
  ...
  if (result < 0) {
    SHADOWHAND_WARN("Cannot get statistics strings for %s : %s",
                    interface_.c_str(), strerror(errno));
    delete[] strings_buf;
    return;
  }
  ...
  unsigned ethtool_stats_buf_len =
      sizeof(struct ethtool_stats) + n_stats_ * sizeof(uint64_t);
  ...
  if (!getEthtoolStats(orig_stats_)) {
    ...
    SHADOWHAND_WARN("Error collecting intial ethernet interface statistics");
    delete[] ethtool_stats_buf_;
    ethtool_stats_buf_ = NULL;
  }
}
...
bool EthernetInterfaceInfo::getInterfaceState(InterfaceState &state) {
  ...
  if (ioctl(sock_, SIOCGIFFLAGS, &ifr) < 0) {
    SHADOWHAND_WARN("Cannot get interface flags for %s: %s", interface_.c_str(),
                    strerror(errno));
    return false;
  }
...
bool EthernetInterfaceInfo::getEthtoolStats(EthtoolStats &s) {
  if (!ethtool_stats_buf_) return false;
  ...
  if (ioctl(sock_, SIOCETHTOOL, &ifr) < 0) {
    SHADOWHAND_WARN("Cannot get NIC stats information for %s : %s",
                    interface_.c_str(), strerror(errno));
    return false;
  }
