...
// -------------------------------------------------------------
#define MOTOR_FLAG_BITS_CURRENT_CHOKE \
  0x000F  //!< Top 4 bits of the current choke.

#define MOTOR_FLAG_BITS_EEPROM_WRITING \
  0x0010  //!< 1=EEPROM write currently in progress, don't update configuration.
#define MOTOR_FLAG_BITS_LAST_CONFIG_CRC_FAILED \
  0x0020  //!< 1=Last CRC message didn't patch the previously sent configs.
#define MOTOR_FLAG_BITS_LAST_CONFIG_OUT_OF_RANGE \
  0x0040  //!< 1=Last config message contained a value which was out of range.
#define MOTOR_FLAG_BITS_JIGGLING_IN_PROGRESS \
  0x0080  //!< Jiggling in progress to zreo gauge readings. Control unavailable
          //!< at this time.

// Serious flags cause the motor to be switched off
// ------------------------------------------------
//#define                                           0x0100
#define MOTOR_FLAG_BITS_MOTOR_ID_IS_INVALID \
  0x0200  //!< Motor seems to have an out-of-range ID. You'll probably never see
          //!< this flag. (Might get rid of it)
#define MOTOR_FLAG_BITS_NO_DEMAND_SEEN \
  0x0400  //!< Haven't received any demand messages for longer than
          //!< NO_DEMAND_TIMEOUT_MS. Motor halted
#define MOTOR_FLAG_BITS_SGL_FAULT \
  0x0800  //!< Fault seen with Left strain gauge. Not currently implemented.

#define MOTOR_FLAG_BITS_SGR_FAULT \
  0x1000  //!< Fault seen with Left strain gauge. Not currently implemented.
#define MOTOR_FLAG_BITS_A3950_NFAULT \
  0x2000  //!< nFault output from A3950 H-Bridge
#define MOTOR_FLAG_BITS_EEPROM_CONFIG_BAD_CRC \
  0x4000  //!< EEPROM contains a bad CRC. Configs not loaded.
...
#define PALM_0200_EDC_NO_DEMAND_TIMEOUT_MS \
  20  //!< If a motor doesn't see any Torque or PWM demand values,
...
typedef enum {
  ...
  MOTOR_CONFIG_FIRST_VALUE =
      0x5,  //!< This is the first TO_MOTOR_DATA_TYPE which is actually a
            //!< configuration and should be stored in EEPROM
  MOTOR_CONFIG_TORQUE_LIMIT = 0x5,  //!< This is the torque value at which the
                                    //!< torque limiter will kick in
  MOTOR_CONFIG_TORQUE_LIMITER_GAIN =
      0x6,  //!< This is the proportional gain term for the torque limit
            //!< controller
...
#define MOTOR_SYSTEM_CONTROL_BACKLASH_COMPENSATION_ENABLE \
  0x0001  //!< Turn on  Backlash Compensation
#define MOTOR_SYSTEM_CONTROL_BACKLASH_COMPENSATION_DISABLE \
  0x0002  //!< Turn off Backlash Compensation
#define MOTOR_SYSTEM_CONTROL_SGL_TRACKING_INC \
  0x0004  //!< Increment the tracking value for Left gauge
#define MOTOR_SYSTEM_CONTROL_SGL_TRACKING_DEC \
  0x0008  //!< Decrement the tracking value for Left gauge
#define MOTOR_SYSTEM_CONTROL_SGR_TRACKING_INC \
  0x0010  //!< Increment the tracking value for Right gauge
#define MOTOR_SYSTEM_CONTROL_SGR_TRACKING_DEC \
  0x0020  //!< Decrement the tracking value for Right gauge
#define MOTOR_SYSTEM_CONTROL_INITIATE_JIGGLING \
  0x0040  //!< Initiate the jiggling to re-zero the strain gauges. You'll
          //!  see the MOTOR_FLAG_BITS_JIGGLING_IN_PROGRESS flag appear.
#define MOTOR_SYSTEM_CONTROL_EEPROM_WRITE \
  0x0080  //!< Write the configuration to the EEPROM.
...
#define MESSAGE_ID_MOTOR_ID_SHIFT_POS \
  5  //!< Bit number of lowest bit of MESSAGE_ID_MOTOR_ID_BITS
#define MESSAGE_ID_MOTOR_ID_BOTTOM_BIT \
  0b00000100000  //!< Used to mask for even/odd motors
#define MESSAGE_ID_MOTOR_ID_TOP_2_BITS \
  0b00110000000  //!< Used to group motors into fours.

#define SENSORS_NUM_0220 \
  ((int)36)  //!< The number of sensors in the robot.
             //!  This needs to be a #define because it's used to dimension an
             //!  array.

#define JOINTS_NUM_0220 \
  ((int)28)  //!< The number of joints in the hand
...
typedef struct {
  EDC_COMMAND EDC_command;  //!< This tells us the contents of the data below.
                            ...
                            //!< EDC_command value which arrived from the host
                            //!< in the previous EtherCAT packet

  int16u sensors[SENSORS_NUM_0220 + 1];

  FROM_MOTOR_DATA_TYPE
  motor_data_type;      //!< Which data does motor[] contain?
  ...
  int32u which_motor_data_arrived;     //!< Bit N set when motor CAN message
                                       //!< arrives. Ideally, bits 0..19 get set
  int32u which_motor_data_had_errors;  //!< Bit N set when motor sends bad CAN
                                       //!< message Ideally, no bits get set.

  MOTOR_DATA_PACKET motor_data_packet[10];  //!< Data for 10 motors only. (Even
                                            //!< ones or Odd ones)
  ...
  int16u
      idle_time_us;  //!< The idle time from when the palm has finished dealing
                     //!< with one EtherCAT packet, and the next packet
                     //!< arriving. Ideally, this number should be more than 50.

} __attribute__((packed)) ETHERCAT_DATA_STRUCTURE_0200_PALM_EDC_STATUS;

//! These are the data sent by the host.
typedef struct {
  EDC_COMMAND EDC_command;  //!< What type of data should the palm send back in
                            //!< the next packet?

  FROM_MOTOR_DATA_TYPE
  from_motor_data_type;  //!< Which data does the host want from the motors?
  ...
  int16s motor_data[NUM_MOTORS];  //!< Data to send to motors. Typically
...
/*
#define ETHERCAT_COMMAND_HEADER_SIZE  (  sizeof(EDC_COMMAND)           \ //!<
What's the minimum amount of
                                       + sizeof(FROM_MOTOR_DATA_TYPE)   \
                                       + sizeof(int16s)  )
*/

#define PALM_0200_ETHERCAT_COMMAND_HEADER_SIZE \
  (sizeof(EDC_COMMAND) + sizeof(FROM_MOTOR_DATA_TYPE) + sizeof(int16s))

#define PALM_0200_ETHERCAT_STATUS_DATA_SIZE \
  sizeof(ETHERCAT_DATA_STRUCTURE_0200_PALM_EDC_STATUS)
#define PALM_0200_ETHERCAT_COMMAND_DATA_SIZE \
...
#define PALM_0200_ETHERCAT_CAN_BRIDGE_DATA_COMMAND_ADDRESS \
  (PALM_0200_ETHERCAT_COMMAND_DATA_ADDRESS +               \
   PALM_0200_ETHERCAT_COMMAND_DATA_SIZE)

#define PALM_0200_ETHERCAT_STATUS_DATA_ADDRESS          \
  (PALM_0200_ETHERCAT_CAN_BRIDGE_DATA_COMMAND_ADDRESS + \
   ETHERCAT_CAN_BRIDGE_DATA_SIZE)
#define PALM_0200_ETHERCAT_CAN_BRIDGE_DATA_STATUS_ADDRESS \
  (PALM_0200_ETHERCAT_STATUS_DATA_ADDRESS + PALM_0200_ETHERCAT_STATUS_DATA_SIZE)
