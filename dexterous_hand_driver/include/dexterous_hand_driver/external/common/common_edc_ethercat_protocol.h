...
typedef enum {
  ...
  EDC_COMMAND_SENSOR_CHANNEL_NUMBERS,  //!< Instead of sending ADC readings,
                                       //!< send the channel number, so the host
                                       //!< can confirm the firmware is correct.
  EDC_COMMAND_SENSOR_ADC_CHANNEL_CS,  //!< Instead of sending ADC readings, send
                                      //!< the chip select channel, so the host
                                      //!< can confirm the firmware is correct.
  EDC_COMMAND_CAN_DIRECT_MODE  //!< Might be used in the future for running
                               //!< automated tests inside the firmware.

  // EDC_COMMAND_TEST_RESULTS,                                   //!< Might be
  // used in the future for running automated tests inside the firmware.
} EDC_COMMAND;
...
typedef enum {
  DIRECTION_DATA_REQUEST = 0x0,  //!< Requesting that motors send back status
                                 //!< data (AKA Start of Frame)
  DIRECTION_TO_MOTOR =
      0x1,  //!< Message contains command data being sent to the motors
  DIRECTION_FROM_MOTOR = 0x2,  //!< Message contains status data from a motor
  DIRECTION_BOOTLOADER =
      0x3  //!< Message has something to do with boot-loading.
} MESSAGE_DIRECTION;

#define MESSAGE_ID_DIRECTION_BITS \
  0b11000000000  //!< Bit mask specifying which bits of the CAN message ID are
                 //!< used for the MESSAGE_DIRECTION
#define MESSAGE_ID_MOTOR_ID_BITS \
  0b00111100000  //!< Bit mask specifying which bits of the CAN message ID are
                 //!< used for the motor ID [0..9]
#define MESSAGE_ID_ACK_BIT \
  0b00000010000  //!< Bit mask specifying which bits of the CAN message ID are
                 //!< used for the ACK bit (only for bootloading)
#define MESSAGE_ID_TYPE_BITS \
  0b00000001111  //!< Bit mask specifying which bits of the CAN message ID are
                 //!< used for the TO_MOTOR_DATA_TYPE or FROM_MOTOR_DATA_TYPE
                 //                                               or for the
                 //                                               TO_MUSCLE_DATA_TYPE
                 //                                               or
                 //                                               FROM_MUSCLE_DATA_TYPE

#define MESSAGE_ID_DIRECTION_SHIFT_POS \
  9  //!< Bit number of lowest bit of MESSAGE_ID_DIRECTION_BITS
...
static const char* joint_names[JOINTS_NUM_0X20] = {
    "FFJ0", "FFJ1", "FFJ2", "FFJ3", "FFJ4", "MFJ0", "MFJ1",
    "MFJ2", "MFJ3", "MFJ4", "RFJ0", "RFJ1", "RFJ2", "RFJ3",
    "RFJ4", "LFJ0", "LFJ1", "LFJ2", "LFJ3", "LFJ4", "LFJ5",
